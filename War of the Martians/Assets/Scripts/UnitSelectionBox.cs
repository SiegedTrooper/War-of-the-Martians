...
public class UnitSelectionBox : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            if (boxVisual.rect.width > 0 || boxVisual.rect.height > 0) {
                UnitSelectionManager.UnitSelector.DeselectAll();
                SelectUnits();
            }
    ...
    void SelectUnits()
    {
        foreach (var unit in UnitSelectionManager.UnitSelector.allUnits)
        {
            if (selectionBox.Contains(myCam.WorldToScreenPoint(unit.transform.position)))
            {
                UnitSelectionManager.UnitSelector.DragSelect(unit);
            }
        }
    }
}